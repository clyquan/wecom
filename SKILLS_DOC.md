...
**注意事项**：
- ✅ 每个文本元素自动独立成段，无需手动添加换行符
- ✅ 每次插入都会重新获取最新文档索引，确保顺序正确
- ✅ 图片会自动上传并插入到对应位置
- ❌ 不要在文本中使用 `\\n` 等手动换行符（可能导致格式异常）
- ❌ 不要使用复杂的格式控制（如 HTML 标签、Markdown 等）

**参数格式验证**：
- ✅ 文本元素：字符串或 `{type: "text", content: "..."}`
- ✅ 图片元素：`{type: "image", url: "https://..."}`
- ❌ 不支持其他类型（如 video、file 等）
- ❌ 不支持嵌套结构（如数组、对象等）
